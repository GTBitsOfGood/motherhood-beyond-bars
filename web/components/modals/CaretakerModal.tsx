            ...
            <div className="block p-6 rounded-lg shadow-lg max-w-md">
              <div className="grid grid-cols-2 gap-4">
                {["firstName", "lastName", "email", "phoneNumber"].map((field, index) => (
                  <div key={field} className={`form-group ${index % 2 === 0 ? "mr-3" : ""} mb-3 ${index === 2 || index === 3 ? "w-full col-span-2" : ""}`}>
                    ...
                    <input
                      ...
                      defaultValue={values[field] ?? null}
                      {...register(field, { required: true })}
                    />
                    {errors[field] && <span className="text-red-500">This field is required</span>}
                  </div>
                ))}
                <div className="form-group flex justify-end w-full col-span-2 mt-3">
                  <button type="button" className="px-4 py-2 rounded-md text-md text-[#B14378] font-semibold" onClick={handleClose}>
                    Cancel
                  </button>
                  <button
                    type="submit"
                    disabled={isSubmitting}
                    className={`bg-transparent hover:bg-[#B14378] text-[#B14378] font-semibold hover:text-white py-2 px-4 border border-[#B14378] hover:border-transparent rounded ${isSubmitting ? "opacity-50 cursor-not-allowed" : ""}`}
                  >
                    {buttonText}
                  </button>
                </div>
                {values?.id && <input type="hidden" defaultValue={values.id} {...register("id")} />}
              </div>
              {isSubmitting && (
